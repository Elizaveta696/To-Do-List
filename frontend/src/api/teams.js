...
export async function fetchTeams() {
  const token = localStorage.getItem("token");
  const res = await fetch("http://localhost:3000/api/teams/all", {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${token}`,
...
export const createTeamTask = async ({ teamId, title, description, dueDate, priority, assignedUserId }) => {
...
export const removeTask = async (teamId, taskId) => {
  const base = resolveBase();
  const token = localStorage.getItem("token");
  const res = await fetch(`${base}/api/teams/${teamId}/tasks/delete/${taskId}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`,
    },
