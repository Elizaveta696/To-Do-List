...
const getTask = async (req, res) => {
	try {
		...
		// Return tasks that are either assigned to the user, or tasks
		// owned by the user that are not assigned to other users. This
		// prevents the personal "My tasks" board from showing tasks the
		// user created but assigned to someone else.
		const tasks = await Task.findAll({
			where: {
				[Op.or]: [{ assignedUserId: userId }, { userId, assignedUserId: null }],
			},
			include: [
				{
					model: Team,
